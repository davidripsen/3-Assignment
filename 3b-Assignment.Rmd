...
However, this system results in non-positive definite state covariance matrix.
...
For this reason, I'm satisfied with model 3. Of course there are yet many possible extensions to try out, but for this 

```{r}
# To assses the model fit, calculate the one-step predictions and residuals. Do residual analysis for model validation.
preds <- predict(fit3, newdata=AllDat)
## Calculate the residuals and put them with the data in a data.frame X
yTi1Hat <- preds$output$pred$yTi1
residuals <- AllDat$yTi1 - yTi1Hat # Hvor kommer q fra?

par(mfrow=c(1,2)) 
acf(fit3$residuals)
pacf(fit3$residuals)

histogram(fit3$residuals)
...
$$
\begin{aligned} d T_{1} &=\frac{1}{C_{i}}\left(\frac{1}{R_{i a}}\left(T_{a}-T_{1}\right)+\frac{1}{R_{i m}}\left(T_{1m}-T_{1}\right)+\Phi+ A_w G_{v}\right) d t+\sigma_{1} d w_{1,1} \\
...
d T_{4} &=\frac{1}{C_{i}}\left(\frac{1}{R_{i a}}\left(T_{a}-T_{4}\right)+\frac{1}{R_{i m}}\left(T_{4m}-T_{4}\right)+\Phi+ A_w G_{v}\right) d t+\sigma_{1} d w_{4,1} \\
...
```{r}
D = AllDat[1:2000,]# Subset the data for computional feasibility
